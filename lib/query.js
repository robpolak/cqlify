...
module.exports = function(instance, opts) {
  ...
  function getInsertParmeters(obj, schema) {
    ...
    for (var property in schema) {
      if (typeof obj[property] !== 'undefined') {
        keys.push(property);
        values.push(_writeCqlType(obj._doc[property], schema[property]));
      }
    }
    return {
      keys: keys.join(','),
  ...
  function update(instance,updateKeys, cb) {
    ...
    instance._connection.client.execute(query, allParams, {prepare: true}, function(err) {
      if(err) {
        if(utility.isFunction(cb))
          cb(err);
      }
  ...
  function insert(instance, cb) {
    ...
    instance._connection.client.execute(query, params.values, {prepare: true}, function(err) {
      if(err) {
        if(utility.isFunction(cb))
          cb(err);
      }
  ...
  function find(instance, queryParams, cb) {
    ...
    instance._connection.client.execute(query, params.values, {prepare: true}, function(err, results) {
      if(err) {
        if(utility.isFunction(cb))
          cb(err, null);
      }
